...
void BridgeScreenWidget::update()
{
    ...
    createAccountButton_->clicked().connect(this, &BridgeScreenWidget::submit); // run login function when button is clicked

}


void BridgeScreenWidget::submit(){

    if (!unsuccessfulPassword_->isHidden())
                unsuccessfulPassword_->setHidden(true);
    if (!unsuccessfulInput_->isHidden())
                unsuccessfulInput_->setHidden(true);
    if (!BridgeScreenWidget::validatePassword()) { // if the password != confirmed password
        unsuccessfulPassword_->setHidden(false); // show user that passwords don't match
      }
    else if (!BridgeScreenWidget::validateInputFields()) {
        unsuccessfulInput_->setHidden(false);
    }
    else { // if password and confirmed password match

        BridgeScreenWidget::writeCredentials(username_->text().toUTF8(), password_->text().toUTF8());
...
int BridgeScreenWidget::writeCredentials(string username, string password) {
  ofstream writefile;
...
bool BridgeScreenWidget::validatePassword() {
    return password_->text() == confirmPassword_->text();
}

bool BridgeScreenWidget::validateInputFields() {
