...
void BridgeScreenWidget::updateBridgeTable(){
    ...
    if(account_->getNumBridges() > 0) {
        ...
        tableRow->elementAt(2)->addWidget(new Wt::WText("IP Address"));
        tableRow->elementAt(3)->addWidget(new Wt::WText("Port"));
        tableRow->elementAt(4)->addWidget(new Wt::WText("Username"));
        tableRow->elementAt(5)->addWidget(new Wt::WText("Status"));
        ...
        for(auto &bridge : account_->getBridges()) {
            ...
            tableRow->elementAt(2)->addWidget(new Wt::WText(bridge.getIP()));
            tableRow->elementAt(3)->addWidget(new Wt::WText(bridge.getPort()));
            tableRow->elementAt(4)->addWidget(new Wt::WText(bridge.getUsername()));
            ...
            string path = "/bridges/" + to_string(counter);
            viewBridgeButton->clicked().connect(boost::bind(&BridgeScreenWidget::viewBridge, this, path));
...
void BridgeScreenWidget::viewBridge(string path) {
